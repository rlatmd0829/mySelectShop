...
class ProductTest {
    ...
    @Nested // 내부의 클래스이다. ex) ProductTest 안에 클래스이다.
    ...
    class CreateUserProduct {
        ...
        @BeforeEach // 각 테스트가 수행되기전에 값을 넣어주고 시작한다.
        void setup() {
            userId = 100L;
        ...
        @Test // 정상케이스 안에 따로 테스트할게 없으므로 클래스로 만들지 않는다.
        @DisplayName("정상 케이스")
        ...
        @Nested // 실패케이스 안에 따로 테스트할 것들이 많으므로 클래스를 만들어 @Nested를 붙여 내부클래스임을 알려준다.
        @DisplayName("실패 케이스")
        class FailCases {
            @Nested
            @DisplayName("회원 Id")
            class userId {
                ...
                void fail1() {
                    ...
                    Exception exception = assertThrows(IllegalArgumentException.class, () -> {
                        new Product(requestDto, userId); // userId가 null이기 때문에 객체를 생성할때 에러가 발생한다.
                    });

                    // then
                    assertEquals("회원 Id 가 유효하지 않습니다.", exception.getMessage());
                    // 에러 발생 message가 예상값이랑 같으면 테스트 성공
                }
